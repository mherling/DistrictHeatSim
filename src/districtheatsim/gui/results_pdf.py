...
def get_custom_table_style():
    ...
    Returns:
        TableStyle: Custom style for tables.
    """
    return TableStyle([
...
def add_figure_to_story(figure, story, max_width=6.5 * inch):
    ...
    Args:
        ...
        story (list): The story list to append the figure to.
        max_width (float, optional): The maximum width for the figure. Defaults to 6.5 * inch.
    """
    img_buffer = BytesIO()
...
def create_pdf(MixDesignTab, filename):
    """
    Creates a PDF with the results of the calculation.

    Args:
        MixDesignTab: The MixDesignTab object containing the data and figures to include in the PDF.
